...
<meta property="og:image" content="{{ "http://" . $_SERVER['HTTP_HOST'] . $event->paralax_img }}"/>
...
<?php $facebookImage = $event->paralax_img; ?>
	<meta property="og:image" content="{{asset($facebookImage or '')}}" />
@endsection
				...
				<div class="container content-tours-desc">
					@if(!empty($event->price))
						<div class="price-wrap text-center">
							<div class="price-price" style="background:#ff4d4d">
								{{ $event->price }}
							</div>
					...
					<div class="row">
						<div class="col-lg-10">
							<ul class="nav nav-tabs">
								@if(isset($tr->body) && !empty($tr->body))
									<li class="active"><a data-toggle="tab" href="#sectionA">{{ trans("events.desc") }}</a></li>
								@endif
								@if(count($event->event_gallery) > 0)
									<li><a data-toggle="tab" href="#sectionC">{{ trans("events.gallery") }}</a></li>
								@endif
								@if(count($event->event_videos) > 0)
									<li><a data-toggle="tab" href="#sectionG">{{ trans("events.videos") }}</a></li>
								@endif
								@if(isset($event->google_map) && !empty($event->google_map))
									<li><a data-toggle="tab" href="#sectionE">{{ trans("events.map") }}</a></li>
								@endif
							</ul>
							<div class="tab-content">
								<div id="sectionA" class="tab-pane fade in active tab-pane-holder">
									<div class="container-fluid body-text-holder">
										@if(isset($tr->body) && !empty($tr->body))
											{!! $tr->body !!}
										@endif
									</div>
										...
										<div class="col-lg-10">
											<div class="container-fluid reviews-wrapper no-padding">
												<div class="row review-holder">
													<h1 class="text-center">{{ trans("events.rew") }} </h1>
													<div class="row comments-wrap">
														<div class="col-lg-10">
															@include("errors/errors")
															<form class="com-form" method="post" action="{{ action('Auth\AuthController@handleComment') }}">
																{{ csrf_field() }}
																<h3>{{ trans("main.ratn") }}</h3>
																<input id="input-rat" data-size="sm" class="rating rating-loading" data-min="0" datshpa-max="5" data-step="1">
																<input type="hidden" value="0" class="hidden-rate" name="user_rate" />
																<input type="hidden" value="event" name="bel_to" />
																<input type="hidden" value="{{ $event->id }}" name="data_id" />
																<div class="form-group com-wrapper">
																	<textarea name="user_comment" class="form-control comment-txt-ar" rows="10"></textarea>
																</div>
																<div class="form-group com-wrapper">
												...
												@if(null !== $event_reviews && count($event_reviews) > 0)
													@foreach($event_reviews as $review)
														<hr/>
														...
														@if(isset($boss_mode) && $boss_mode === true)
															<button type="button" role="button" class="btn btn-default btn-danger" data-comment-id="{{ action("AdminEventsController@removeCommment", ["id" => $review->id]) }}">წაშლა</button>
														@endif
													@endforeach
												@endif
											</div>
						...
						@include("ad-show", ["class" => "ad-small ad-evn"])
					</div>
			@endif
		@endforeach
	@endif
...
<script type="text/javascript">
	$(document).ready(function(){
		$('.parallax-window').parallax({imageSrc: "{{ asset($event->paralax_img) }}"});
		...
		if($('.btn-comment').length > 0){
			$('.btn-comment').on('click', function(event) {
				...
				$.ajax({
					...
					url: "{{ action("Auth\AuthController@handleComment", ["id" => $event->id]) }}",
					method: 'POST',
					dataType: 'json',
