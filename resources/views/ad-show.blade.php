@if(isset($ads) && count($ads) > 0)
    @if(isset($class))
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-2 {{ $class }} ad-main-style" style="padding:30px 0 0 0; margin-top: 33px; background-color: transparent;">
            @else
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-2 ad-main-style" style="padding:30px 0 0 0; margin-top: 33px; background-color: transparent;">
                    @endif
                    @foreach($ads as $ad)
                        @if(isset($ad->event_id))
                            @foreach($ad->event->event_trans as $trans)
                                @if($trans->lang->lang == App::getLocale())
                                    <div class="col-xs-offset-2 col-xs-8 col-sm-offset-2 col-sm-4 col-md-3 col-lg-12 new-commerc">
                                        <a href="{{ action("EventsController@show", ['id' => $ad->event_id]) }}" class="ad-main-link" style="height:100%">
                        ...
                        @elseif(isset($ad->tour_id))
                            @foreach($ad->tour->tour_trans as $trans)
                                @if($trans->lang->lang == App::getLocale())
                                    <div class="col-xs-offset-2 col-xs-8 col-sm-offset-2 col-sm-4 col-md-3 col-lg-12 new-commerc">
                                        <a href="{{ action("ToursController@show", ['id' => $ad->tour_id]) }}" class="ad-main-link" style="height:100%">
                                            <div class="img-parent view view-sixth">
